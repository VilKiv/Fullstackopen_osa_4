...
describe('Most blogs', () => {
    ...
    test('when list has only one author they have also the most blogs', () => {
        ...
        assert.strictEqual(JSON.stringify(result), JSON.stringify(mostInSingleBlog))
    })

    test('Favourite from multiple blogs', () => {
        const result = listHelper.mostBlogs(listWithMultipleBlogs)
        assert.strictEqual(JSON.stringify(result), JSON.stringify(MostInMultipleBlogsList))
    })
})