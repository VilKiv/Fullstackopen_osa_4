...
describe('favourite blogs', () => {
    ...
    test('when list has only one blog it\'s also the most liked', () => {
        ...
        assert.strictEqual(JSON.stringify(result), JSON.stringify(listWithOneBlog[0]))
    })

    test('Favourite from multiple blogs', () => {
        const result = listHelper.favouriteBlog(listWithMultipleBlogs)
        assert.strictEqual(JSON.stringify(result), JSON.stringify(listWithMultipleBlogs[2]))
    })
})